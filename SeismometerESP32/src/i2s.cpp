...
void I2S_Task::Run() {
    ...
    uint16_t* single_channel_buff = (uint16_t*)malloc(sizeof(uint16_t)*I2S_SAMPLES_MONO_FRAME);
    if(single_channel_buff == NULL){
        ESP_LOGE(TAG, "FAILED BUFFER ALLOCATION");
        vTaskDelete(NULL);
    }
    uint8_t* i2s_adc_isr_buffer = (uint8_t*)malloc(I2S_DMA_BUFFER_SIZE);
    if(i2s_adc_isr_buffer == NULL){
        ESP_LOGE(TAG, "FAILED BUFFER ALLOCATION");
        vTaskDelete(NULL);
    }
    uint8_t* i2s_adc_raw_buffer = (uint8_t*)malloc(I2S_DMA_BUFFER_SIZE);
    ...
    while (1) {       
        ...
        for (size_t j = 0; j < (I2S_DMA_BUFFER_SIZE/6); j++) {
            ...
            single_channel_buff[j] = channel_1_value;
            ...
            //printf("%ld\t%ld\n", (int32_t)(volume_1*999.0f), (int32_t)(volume_2*999.0f));
        }
        ...
        xQueueSend(System::i2s_to_dsp_queue.getHandle(), (void*)&package, 0);
    }
}
